...
- `MODEL_ROLLBACK_ARCHIVE_ID`, `MODEL_ROLLBACK_CREATED_BY`, `MODEL_ROLLBACK_REASON` 값을 Docker 컨테이너 내부로 전달하도록 Makefile target 개선
...
```text
Alertmanager silence 생성/해제 이력 관리 개선
Synthetic incident response drill 추가
Prometheus alert rule unit test 추가
Prometheus rule test 파일을 tests 디렉터리로 분리
FastAPI /health / /ready endpoint 추가
API readiness metrics 추가
JobSkillApiNotReady alert rule 추가
API not ready runbook 추가
Alert workflow smoke check에 readiness metric 검증 추가
Runbook coverage check 추가
Metrics contract check 추가
Alert rule metric dependency check 추가
Static ops validation check 추가
GitHub Actions Smoke Check에 static ops validation 단계 추가
Ops validation check command 고도화
Makefile help / validation command 정리
Model Promotion Archive 기능 추가
Model Rollback CLI 추가
model_promotion_archives / model_rollback_actions 테이블 추가
model-archive / model-rollback-plan / model-rollback 명령어 추가
rollback dry-run plan과 없는 archive id 검증 확인
...
Streamlit Dashboard에서 archive 목록 조회 / rollback plan / rollback 실행 UI 추가
rollback 이후 FastAPI /reload-model 호출 자동화
rollback smoke check 또는 model lifecycle check 추가
Data Contract Check로 raw/cleaned/model prediction schema 검증
