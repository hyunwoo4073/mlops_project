...
```text
MLflow Training Dataset Tracking 추가
training_dataset_profile.json artifact 저장
training_dataset_hash 기반 학습 데이터 추적 추가
MLflow Dataset API 기반 mlflow.log_input 기록 추가
Model Evaluation Artifacts 추가
classification_report.json / csv / txt 저장
confusion_matrix.csv 저장
evaluation_distribution.json 저장
class별 precision / recall / f1-score / support metric 저장
Promoted Model Card 생성 기능 추가
reports/latest_model_card.md 생성
reports/model_cards/model_card_registry_{id}.md 생성
Model Card에 MLflow run / dataset profile / evaluation artifact / lifecycle 정보 포함
Class-level Model Performance Gate 추가
MODEL_CLASS_PERFORMANCE 결과를 pipeline_check_results에 저장
Airflow DAG에 check_model_class_performance task 추가
Airflow DAG에 generate_model_card task 추가
Streamlit Dashboard에 Model Evaluation 탭 추가
class별 support / recall / f1 / threshold / pass-fail 상태 시각화
Streamlit Dashboard에 Model Card 탭 추가
Dashboard에서 Model Card 생성 / 미리보기 / 다운로드 기능 추가
Dashboard tab routing 오류 수정
Docker Compose dashboard 서비스에 reports volume mount 추가
Makefile에 model-class-performance-check / model-card 명령어 추가
README에 model evaluation governance workflow 반영
```

## 다음 개선 예정

```text
Model Card validation check 추가
Model Card와 현재 PROMOTED model_registry / MLflow run_id 정합성 검증
