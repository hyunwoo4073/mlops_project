...
데이터 품질 체크, 모델 성능 체크, prediction quality gate, prediction drift gate 결과를 PostgreSQL의 `pipeline_check_results` 테이블에 저장합니다.
...
```text
Model Promotion Archive 기능 추가
Model Rollback CLI 추가
Model Rollback CLI dry-run / archive id 전달 검증
Docker Compose exec 환경변수 전달 방식 수정
Streamlit Dashboard Model Lifecycle 탭 추가
Current promoted model / archive 이력 / rollback action 이력 조회 기능 추가
Rollback Plan Helper 추가
Incident Report 탭 routing 오류 수정
Model Lifecycle Integrity Check 추가
model-lifecycle-check 명령어 추가
ops-check에 model lifecycle integrity check 포함
Data Contract Check 추가
config/data_contract.json 기반 데이터 계약 정의
check_data_contract.py 기반 필수 테이블/컬럼/type/null/empty/allowed value 검증 추가
data-contract-check 명령어 추가
Docker Compose에 config 디렉터리 mount 추가
Airflow DAG에 check_data_contract task 추가
preprocess_jobs → check_data_contract → check_training_data 흐름 적용
job_post_skills schema를 job_post_id / skill_name 기준으로 정리
job_post_skills는 optional 산출물로 보고 min_rows=0 적용
DATA_CONTRACT 검증 결과를 pipeline_check_results에 저장
README에 model lifecycle dashboard / integrity check / data contract check 반영
```
...
Data Contract Check를 GitHub Actions smoke workflow에 포함
Model Lifecycle Dashboard에 archive file existence 상태 표시
Model Rollback Dashboard Action 추가
rollback 이후 FastAPI /reload-model 호출 자동화
rollback smoke check 고도화
