...
FastAPI /health 확인
FastAPI /ready 확인
FastAPI /metrics readiness metric 확인
주요 runbook 파일 존재 확인
...
`make ops-check`는 Prometheus rule test, Alertmanager config check, FastAPI health/readiness, runbook 존재 여부, smoke check, alert workflow smoke check를 한 번에 검증합니다.
...
Ops validation check command 추가
Makefile help / validation command 정리
```
