...
│   ├── alert_acknowledgements / alert_settings 저장
...
`ALERT_MAINTENANCE_MODE`는 `alert_settings` 테이블이 없거나 초기화되지 않은 경우에 사용하는 fallback 값입니다. 정상 운영 중에는 Streamlit Dashboard에서 변경한 `alert_settings.maintenance_mode` 값이 우선 사용됩니다.
