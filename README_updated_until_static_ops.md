...
```text
Model Promotion Audit / Rollback 기능 추가
이전 PROMOTED 모델로 되돌리는 rollback script / dashboard action 추가
model_registry에 rollback action 이력 저장
rollback 이후 FastAPI serving model reload 검증
