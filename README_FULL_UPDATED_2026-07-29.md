...
- 최근 production feedback, 오분류 목록, actual/predicted confusion table, feedback source 분포, latest evaluation check 결과 조회 기능 추가
- Dashboard에서 최근 prediction을 선택해 `actual_category`, `feedback_source`, `feedback_note`, `created_by`를 입력하고 `prediction_feedbacks`에 upsert하는 Feedback Input 기능 추가
- Dashboard에서 `PRODUCTION_FEEDBACK` 평가를 직접 실행하는 Evaluation Runner 기능 추가
- Dashboard 기반 평가 실행 결과를 `pipeline_check_results`에 `PRODUCTION_FEEDBACK` 타입으로 저장
- `Evaluation History` 탭을 추가해 production feedback 평가 이력을 시간 흐름으로 확인하도록 개선
- Evaluation History에서 feedback count, accuracy, weighted F1, status distribution, raw evaluation history를 확인할 수 있도록 구성
- `Retraining Candidate` 탭을 추가해 production feedback 기반 재학습 후보 여부를 Dashboard에서 판단하도록 개선
- 재학습 후보 판단 기준으로 feedback 수, 최신 accuracy, 최신 weighted F1, 최신 check status, 최근 accuracy/F1 하락 추세를 사용
- Retraining Candidate 탭에서 판단 근거, 권장 조치, 최근 오분류 샘플, 오분류 집중 패턴을 확인하도록 구성
- Streamlit `number_input` 중복 ID 오류를 방지하기 위해 Retraining Candidate 위젯에 고유 `key` 적용
- Dashboard에서 재학습 후보 판단 결과를 `pipeline_check_results`에 `RETRAINING_CANDIDATE` 타입으로 저장하는 기능 추가
- `retraining_candidate_flag`, `retraining_feedback_count`, `retraining_accuracy`, `retraining_f1_weighted`, `retraining_accuracy_delta`, `retraining_f1_delta` check 결과 저장
- FastAPI `/metrics`에 retraining candidate 관련 metric 추가
- `/metrics`에 `jobskill_retraining_candidate_flag`, `jobskill_retraining_candidate_feedback_count`, `jobskill_retraining_candidate_accuracy`, `jobskill_retraining_candidate_f1_weighted`, `jobskill_retraining_candidate_accuracy_delta`, `jobskill_retraining_candidate_f1_delta` 노출
- `monitoring/metrics_contract.yml`에 retraining candidate metric contract 추가
- `JobSkillRetrainingCandidateDetected` Prometheus alert rule 추가
- 재학습 후보 감지 대응 runbook `docs/runbooks/jobskill_retraining_candidate_detected.md` 추가
- Prometheus rule test에 retraining candidate firing 및 maintenance mode suppression 케이스 추가
- `runbook-check`, `prometheus-check`, `prometheus-rule-test`, `alert-rule-metric-check`, `metrics-contract-check`, `ops-static-check` 검증 흐름 통과 기준 정리
...
- Alertmanager Slack notification 전송 실패를 `alertmanager_notifications_failed_total` metric으로 감지
- `JobSkillAlertmanagerNotificationFailure` Prometheus alert rule 추가
- Alertmanager notification failure 대응 runbook `docs/runbooks/jobskill_alertmanager_notification_failure.md` 추가
- Alertmanager notification failure rule에 `runbook_url`, `dashboard_url`, `prometheus_url` annotation 연결
- Prometheus rule test에 `alertmanager_notification_failure_should_fire` 케이스 추가
- `monitoring/metrics_contract.yml`에 `external_metrics.alertmanager` 구조 추가
- FastAPI `/metrics` 전용 required metric과 Alertmanager 외부 metric을 분리 관리
- `scripts/check_metrics_contract.py`가 FastAPI metric과 external metric source를 함께 검증하도록 개선
- `scripts/check_alert_rule_metric_dependencies.py`가 multi-source metric contract를 인식하도록 개선
- alert rule dependency check에서 FastAPI metric은 FastAPI `/metrics`, Alertmanager metric은 Alertmanager `/metrics` 기준으로 검증
- PromQL `sum by (receiver, integration)`의 grouping label을 metric으로 오인하지 않도록 parser 개선
- label 없는 Prometheus metric line도 정상 파싱하도록 regex 수정
- `src/monitoring/prometheus_metrics.py`에서 query 결과가 비어도 required metric을 0 값으로 노출하도록 `_add_metric()` 안정화
- DB 초기화 직후에도 `jobskill_alert_acknowledgements_total`, `jobskill_alert_avg_mtta_minutes`, `jobskill_alert_unacknowledged_current_total`이 0으로 노출되도록 개선
- `runbook-check`, `metrics-contract-check`, `alert-rule-metric-check`, `prometheus-rule-test` 정상 동작 확인
- README가 길어짐에 따라 상세 README, 요약본, Quick Start 문서를 분리 관리하는 문서 구조 정리
...
Production Feedback Dashboard는 운영 예측에 연결된 feedback을 Streamlit에서 조회, 입력, 평가, 이력화, 재학습 후보 판단까지 수행하기 위한 운영 화면입니다.

기존 Production Feedback Evaluation Loop가 CLI와 Prometheus metric 중심이었다면, Dashboard 개선 이후에는 운영자가 직접 화면에서 feedback을 입력하고 평가 결과를 확인할 수 있습니다.

```text
Prediction 생성
→ Dashboard에서 actual_category feedback 입력
→ prediction_feedbacks upsert
→ Dashboard에서 production feedback evaluation 실행
→ PRODUCTION_FEEDBACK 결과 저장
→ Evaluation History에서 추세 확인
→ Retraining Candidate 탭에서 재학습 후보 판단
→ RETRAINING_CANDIDATE 결과 저장
→ /metrics 노출
→ Prometheus alert / runbook 대응
```

### Production Feedback 탭

`src/dashboard/app.py`

Streamlit Dashboard에 `Production Feedback` 탭을 추가했습니다.
...
Recent Feedback
Wrong Predictions
Actual Category vs Predicted Category Confusion Table
Feedback Source Distribution
Latest PRODUCTION_FEEDBACK Check Results
```
...
Dashboard에서 최근 prediction을 선택해 실제 라벨 feedback을 저장하거나 수정할 수 있습니다.
...
```text
prediction_feedbacks에 INSERT
이미 같은 prediction_id가 있으면 UPDATE
```

이를 통해 CLI 샘플 생성뿐 아니라 운영자가 직접 feedback을 입력할 수 있습니다.

### Evaluation Runner

Dashboard에서 production feedback 평가를 직접 실행할 수 있습니다.
...
실행 결과는 `pipeline_check_results`에 `PRODUCTION_FEEDBACK` 타입으로 저장됩니다.
...
Production Feedback 평가 이력을 시간 흐름으로 확인합니다.
...
Latest Accuracy
Latest Weighted F1
Accuracy / Weighted F1 Trend
Feedback Count Trend
Evaluation Status Distribution
Raw Evaluation History
```

이를 통해 단일 평가 결과뿐 아니라 최근 평가마다 모델 품질이 좋아지는지, 나빠지는지 확인할 수 있습니다.

## Retraining Candidate Operations Loop

Retraining Candidate Operations Loop는 production feedback 평가 결과를 기반으로 현재 promoted model이 재학습 후보인지 판단하고, 그 판단 결과를 저장·노출·알림으로 연결하는 기능입니다.

```text
Production Feedback 평가 이력
→ feedback 수 충분성 확인
→ accuracy / weighted F1 기준 확인
→ 최근 평가 trend 확인
→ 오분류 집중 패턴 확인
→ 재학습 후보 여부 판단
→ RETRAINING_CANDIDATE 결과 저장
...
### Retraining Candidate Dashboard

`Production Feedback → Retraining Candidate` 탭에서 현재 모델의 재학습 후보 여부를 판단합니다.
...
```text
feedback 수가 충분한지
최신 production accuracy가 기준보다 낮은지
최신 weighted F1이 기준보다 낮은지
최신 PRODUCTION_FEEDBACK check가 FAIL인지
최근 evaluation history에서 accuracy/F1이 하락 중인지
오분류가 특정 actual/predicted 조합에 집중되는지
```

Dashboard에서 확인 가능한 항목:
...
최근 오분류 샘플
오분류 집중도
최근 RETRAINING_CANDIDATE 저장 이력
```

### Retraining Candidate 결과 저장

Dashboard의 `Save retraining candidate decision` 버튼을 클릭하면 판단 결과가 `pipeline_check_results`에 저장됩니다.
...
```text
PASS    : 현재 기준에서는 재학습 후보가 아님
FAIL    : 현재 기준에서 재학습 후보임
SKIPPED : feedback 수가 부족해 판단 보류
```
...
즉, 점검 모드가 아니고 최신 재학습 후보 판단 결과가 candidate 상태이면 alert가 발생합니다.

연결 runbook:

```text
docs/runbooks/jobskill_retraining_candidate_detected.md
```
...
│   │   └── jobskill_alertmanager_notification_failure.md
│   └── images/
...
FastAPI `/metrics`와 `external_metrics`에 정의된 외부 metric source가 운영상 필요한 Prometheus metric을 안정적으로 노출하는지 검증합니다.
...
### 16-20-1. Alertmanager Notification Failure Monitoring

`monitoring/prometheus/rules/jobskill_alert_rules.yml`  
`docs/runbooks/jobskill_alertmanager_notification_failure.md`  
`monitoring/prometheus/tests/jobskill_alert_rules.test.yml`

Alertmanager가 Slack receiver로 알림을 전송하지 못하는 상황을 별도 alert로 감지합니다.
...
```promql
sum by (receiver, integration) (increase(alertmanager_notifications_failed_total[5m])) > 0
```

이 alert는 Prometheus alert rule이 정상 firing 되었더라도 Slack 알림 전송이 실패할 수 있는 상황을 감지하기 위한 알림 채널 자체의 모니터링입니다.
...
### 16-20-2. External Metrics Contract Check

`monitoring/metrics_contract.yml`  
`scripts/check_metrics_contract.py`

`metrics_contract.yml`에서 FastAPI가 노출하는 application metric과 외부 component metric을 분리해서 관리합니다.
...
`required_metrics`는 FastAPI `/metrics`에서 확인하고, `external_metrics.alertmanager.required_metrics`는 Alertmanager `/metrics`에서 확인합니다.
...
이를 통해 Prometheus alert rule, Grafana dashboard, ops validation에서 사용하는 metric이 실제 endpoint에 노출되는지 source별로 검증합니다.

### 16-20-3. Multi-source Alert Rule Metric Dependency Check

`scripts/check_alert_rule_metric_dependencies.py`  
`monitoring/metrics_contract.yml`

Prometheus alert rule이 참조하는 metric이 올바른 metric source에 존재하는지 검증합니다.

검증 방식:

```text
jobskill_* metric
→ FastAPI /metrics 기준 확인

alertmanager_* metric
→ Alertmanager /metrics 기준 확인

up
→ Prometheus scrape built-in metric으로 external 처리

receiver / integration
→ PromQL grouping label이므로 metric dependency에서 제외
```
...
이 검증은 alert rule이 존재하지 않는 metric을 참조하거나, 잘못된 source의 metric을 바라보는 문제를 방지합니다.

### 16-20-4. Stable Zero Metrics Exposure

`src/monitoring/prometheus_metrics.py`

DB 초기화 직후처럼 query 결과가 비어 있는 상황에서도 운영상 필수 metric은 0 값으로 항상 노출되도록 개선했습니다.
...
Prometheus metric contract 관점에서는 값이 0인 것과 metric이 아예 없는 것이 다릅니다. 이 개선으로 DB row가 아직 없더라도 metric contract check와 alert rule dependency check가 안정적으로 동작합니다.
...
`make metrics-contract-check`는 FastAPI `/metrics`와 external metric source가 운영 필수 metric을 노출하는지 검증합니다.

```bash
make metrics-contract-check
```

`make alert-rule-metric-check`는 Prometheus alert rule이 참조하는 metric이 metrics contract와 각 metric source의 `/metrics`에 존재하는지 검증합니다.
